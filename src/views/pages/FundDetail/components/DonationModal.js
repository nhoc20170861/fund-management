...
const checkAccountBalance = async (projectWalletAddress) => {
    try {
        const accountInfo = await algodClient.accountInformation(projectWalletAddress).do();
        const balance = accountInfo.amount; // Số dư tài khoản trong microAlgos
        const balanceInVND = convertMicroAlgosToVND(balance);
        console.log(`Số dư tài khoản ${projectWalletAddress}: ${balance} microAlgos (${balanceInVND} VND)`);
        setProjectDetail((prev) => ({...prev, current_fund: balanceInVND}));
        return balanceInVND;
    } catch (error) {
    ...
    const sendTransaction = async () => {
        ...
        console.log("🚀 ~ sendTransaction ~ projectWalletAddress:", projectWalletAddress);
        ...
        try {
            ...
            console.log("🚀 ~ addContributeTranstaction ~ newContribute_trans:", newContribute_trans);

            try {
                const response = await addContributeTranstaction(projectId, newContribute_trans);
